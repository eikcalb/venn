...
final class AssetManager {
    ...
    private function get_file($file, $namespace = null) {
        if (!empty($namespace)) {
            $file = $namespace . substr($file, 0, 1) === DIRECTORY_SEPARATOR ? $file : DIRECTORY_SEPARATOR . $file;
        }
        if (is_readable(SERVICE_ROOT_DIRECTORY . $file) && false != $data = file_get_contents(SERVICE_ROOT_DIRECTORY . $file)) {
            return $data;
        } else {
            return false;
        }
